...
def sample_convert(text1, text2):
    ...
    text1_ids, out1_ids = random_ngram_mask(text1_ids)  # 如果不用ngram mask，直接把random_ngram_mask换成random_mask就是动态随机mask了
...
warmup = WarmUpLearningRateScheduler(warmup_batches=int(len(train_generator) * epochs * 0.1), init_lr=learning_rate)
...
model_saved_path = '/content/drive/MyDrive/textSim/bert_result/nezha-pre-ngram.ckpt'
class ModelCheckpoint(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if logs['loss'] < self.loss:
            self.loss = logs['loss']
            self.model.save_weights('/content/drive/MyDrive/textSim/bert_result/'+'nezha-pretrain-ngram.weights')
...
train_model.fit(
    ...
    epochs=epochs,
    callbacks=[warmup,checkpoint, csv_logger],
)